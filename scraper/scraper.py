...
def status(msg):
    try:
        producer = SimpleProducer(client, async = True, batch_send_every_n = 1000, batch_send_every_t = 1)
        producer.send_messages(b'news', msg)
        print "produced!"
...
def get_rss(address, website):
    ...
    try:
        ...
        logger.warning('Problem fetching RSS feed for {}. {}'.format(address, e))
        results = None
    ...
    for result in rss_results:
        ...
        if text:
            ...
            if(is_atrocity(cleaned_text)):
                ...
                print cleaned_text
                status(cleaned_text)
                entry_id = mongo_connection.add_entry(db_collection, cleaned_text,result.title, result.url,result.date, website, lang)
                if entry_id:
                    try:
...
def is_atrocity(text):
    atrocity_words = ['killed', 'kills', 'massacre', 'bomb', 'bombing', 'bomber', 'beheaded','mass grave','murder','assassinate']
    _atrocity_words = ['earthquake', 'accident','floods', 'hurricanes', 'tornadoes', 'volcanic eruptions', 'earthquakes', 'tsunamis']
    text_tokens = text.split()
    if(not set(atrocity_words).isdisjoint(text_tokens)):
        if(set(_atrocity_words).isdisjoint(text_tokens)):
            return True
    return False
